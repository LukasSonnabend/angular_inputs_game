...
export class AnimalMinimalFormComponent implements OnInit {
  ...
  constructor(
    protected monsterSelectionService: MonsterSelectionService,
  ...
  ngOnInit() {
    this.monsterSelectionService
      .getSelectedMonstersObservable()
      .subscribe((selectedMonsters) => {
        // Initialize an empty form group
        this.form = this.fb.group(
          {},
          {
            validators: [
              genderMismatchValidator,
              speciesMismatchValidator,
              maturityValidator,
            ],
          }
        );
        ...
        if (selectedMonsters.length > 0 && selectedMonsters[0] !== null) {
          this.form.addControl(
            "animalInput1",
            new FormControl(selectedMonsters[0])
          );
        }
        ...
        if (selectedMonsters.length > 1 && selectedMonsters[1] !== null) {
          this.form.addControl(
            "animalInput2",
            new FormControl(selectedMonsters[1])
          );
        }
  ...
  onSubmit() {
    this.breedingService.submitBreedingPair(
      this.form?.value.animalInput1,
      this.form?.value.animalInput2
    );
