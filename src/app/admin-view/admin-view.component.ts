    ...
    <div class="star-wars-theme">
      ...
      <div>
        <label
          ...
          >Select a Monster:</label
        >
        <select
          id="monsterSelect"
          (change)="onMonsterSelected($event)"
          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        >
          <option disabled selected value>-- select an option --</option>
          <option *ngFor="let monster of monsters" [value]="monster.name">
            {{ monster.name }}
          </option>
        </select>
      </div>
      <form [formGroup]="monsterForm" (ngSubmit)="onSubmit()">
        <div *ngIf="selectedMonster">
          <div class="mt-4 flex">
            ...
            >
            <div
              *ngFor="let stage of evolutionStages"
            ...
            >
              <input
                ...
                [id]="stage"
                [value]="stage"
                formControlName="evolutionStage"
                class="mr-2"
              />
      ...
      <div *ngIf="createdMonsters.length > 0" class="mt-4">
        <button
          ...
          (click)="addMonstersToAnimalService()"
        >
          Monster speichern
        </button>
...
export class AdminViewComponent implements OnInit {
  ...
  onMonsterSelected(e: Event): void {
    ...
    this.selectedMonster = this.monsters.find(
  ...
  onSubmit(): void {
    if (this.monsterForm.valid) {
      ...
      for (let i = 0; i < this.monsterForm.controls["quantity"].value; i++) {
        this.createdMonsters.push({
          ...generateNewMonsterW(
            this.monsterForm.controls["species"].value,
            this.monsterForm.controls["evolutionStage"].value,
            this.monsterForm.controls["gender"].value
          ),
          uuid: uuidv4(),
        });
      }
