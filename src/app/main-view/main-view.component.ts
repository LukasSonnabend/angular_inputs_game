...
@Component({
  standalone: true,
  template: `<button class="mini-button button-primary" (click)="buttonClicked()">{{label}}</button>`,
})
...
@Component({
    ...
    <div class="flex justify-end">
      <div class="flex gap-10">
        <button  class="button button-primary>" (click)="saveToSupabase()">save</button>
        <button class="button button-secondary>" (click)="loadFromSupabase()">load</button>
    </div>
  </div>
  <div>
  <form ngForm>
  <label>
    Species:
    <select [(ngModel)]="selectedSpecies" (change)="filterAnimals()" name="species">
      <option value="">All</option>
      <!-- Add an option for each species -->
      <option *ngFor="let species of availableMonstersAsOptions()" [value]="species.value">{{ species.label }}</option>
      <!-- etc. -->
    </select>
  </label>
  <label>
    Growth Stage:
    <select [(ngModel)]="selectedGrowthStage" (change)="filterAnimals()" name="growthStage">
      <option value="">All</option>
  ...
  <button (click)="previousPage()" class="button button-primary">Previous page</button>
<button (click)="nextPage()">Next page</button>

<div class="ag-theme-balham">
<ag-grid-angular
 style="height: 500px; width: 100%;" class="ag-theme-balham-dark"
...
`,
    imports: [AnimalFormComponent, AttributeInputComponent, CommonModule, MonsterCardComponent, BreedingPodListComponent, FormsModule, ReactiveFormsModule, AgGridAngular]
})
export class MainViewComponent implements OnInit {
  ...
  { headerName: 'Delete', maxWidth: 75, cellRenderer: CustomButtonComponent, cellRendererParams: { onClick: (e: DnDMonster) => this.deleteAnimal(e.uuid) }},
    ...
    { headerName: 'Gestation Period', field: 'nerfed.gestationPeriod.enumValue' },
    { headerName: 'Stärke', field: 'nerfed.strength.enumValue', maxWidth: 125},
    { headerName: 'Remarkability', field: 'nerfed.remarkability.enumValue', maxWidth: 125},
    { headerName: 'Mutation Chance', field: 'nerfed.mutationChance.enumValue', maxWidth: 130},
  ...
  filterAnimals(): void {
    const allFilteredAnimals = this.animals.filter(animal => {
      return (this.selectedSpecies === '' || animal.species.name === this.selectedSpecies) &&
             (this.selectedGrowthStage === '' || animal.evolutionStage === this.selectedGrowthStage);
    });

    // Slice the array to get only the animals for the current page
    this.filteredAnimals = allFilteredAnimals.slice(this.pageIndex * this.pageSize, (this.pageIndex + 1) * this.pageSize);
  }
